...
import uuid
import logging
from backend.core.events import housekeeping_loop
...
async def add_security_headers(request: Request, call_next):
    ...
    # Legacy XSS filter (kept for defence-in-depth, though deprecated by most browsers)
    ...
    # Content Security Policy (restrictive default for API; relax in frontend nginx)
...
async def health_check() -> Dict[str, Any]:
    ...
    Returns current UTC timestamp and a basic status indicator.
    Extend this to validate DB connectivity, Redis, Celery workers, etc.
    """
    # os.getlogin() removed: it fails in containers, CI, or under certain service accounts.
    return {
        "status": "healthy",
...
if __name__ == "__main__":
    uvicorn.run(
        ...
        access_log=False,  # we handle access logging manually via middleware
    )
