...
async def event_stream(
    ...
    for ev in history:
        yield _sse_line(ev)
        if ev.get("event") == "status" and ev.get("data", {}).get("status") in _TERMINAL_STATUSES:
            return
...
async def housekeeping_loop() -> None:
    """Background coroutine that purges stale scan buffers every 30 seconds."""
    while True:
        ...
        if stale:
            logger.debug("Housekeeping evicted %s stale scan buffers", len(stale))
